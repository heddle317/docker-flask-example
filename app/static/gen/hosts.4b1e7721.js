...
angularApp.controller('RunScriptController', ['$scope', '$http', '$window', '$log', function($scope, $http, $window, $log) {
    $scope.newScript = {'name': '',
                        'args': []};
    $scope.currentArg = '';
    $scope.getScripts = function() {
        var $btn = $('.btn.get-scripts').button('loading');
        $http.get('/api/services/' + $scope.serviceUuid + '/scripts', {params: {'password': $scope.password}}).success(function(response) {
            $scope.editErrorMessage = '';
            $scope.scripts = response;
            $log.log($scope.scripts);
